...
- 调查基线为 `main@3c869227071a762abc76394584d72aa80d329da8`，GitHub/NJU `main` 一致；仓库无 tag。设计与执行计划必须先通过普通 PR 合并并经双端 CI，清理执行以该合并后的新 `main` 为唯一基线。
...
强制更新远端前，从原始 `main@3c86922` 创建 Git bundle，存放于工作区外的 OS 临时目录。备份文件只用于本次失败恢复，不上传、不提交、不复制到项目目录。
