...
> 日期：2026-08-08
>
> 状态：用户已于 2026-08-08 明确批准；`PLAN.md` 生成后须完成陌生智能体冷启动，之后才能编写实现代码
...
| 行为 | 会话内始终记录；跨会话存储默认关闭，启用后只保存项目说明和成功摘要；使用版本 1 的四字段白名单 JSON、同目录临时文件和原子替换；按需检索后注入 ContextBuilder |
| 输出 | 精简 ProjectMemory 或内存状态 |
| 边界 | 模型不能任意写入记忆；项目说明和成功摘要各最多 2,000 字符；拒绝秘密字段/令牌、完整对话、文件全文、失败恢复内容、保护树和静态 reparse 路径 |
| 错误 | 格式损坏或超限时仅在成功隔离后回退并给出固定无秘密提示；不可读、隔离失败或路径不安全时禁止覆盖；写入失败不影响文件事务 |
...
本 harness 的安全边界只由三部分构成：工作区路径围栏、动作级策略分类与 HITL、逐文件事务与回滚。pytest 会以当前用户权限执行工作区内代码；这类代码执行明确不在安全边界内。capability 扫描只是对常见模式抬起 `CONFIRM` 的启发式提示层，不完备，也不构成安全保证。完备隔离需要容器、独立虚拟机或系统级沙箱，属于本项目非目标。
...
早期原型/检测脚本曾将额度受限的临时 API Key 写入 Git 历史。用户于 2026-08-12 明确授权精确重写本人独占仓库；项目使用 `git-filter-repo` 在一次性 mirror 中替换全部历史 `sk-...` 模式，保持原 head tree 不变，并以带明确旧 SHA 的 `force-with-lease` 更新 GitHub/NJU。重写后当前树扫描、完整历史扫描、全量测试和 Ruff 均通过。规范分支已满足“历史中不得出现凭据”的门禁；GitHub cached views/internal PR refs 仍属于平台 Support 的外部清理边界。
...
| R-09 | 路径式工作区围栏存在本地并发替换（TOCTOU）窗口 | 同一 Windows 用户下的其他进程若在检查与读取之间恶意替换文件或目录，应用层围栏不能提供操作系统沙箱级保证 | 用户选择保留路径式方案；通过重解析点检查、resolve/commonpath、打开前后文件身份/大小/时间比较和事务哈希降低误操作风险，并明确不把它描述为对抗本地恶意进程的安全边界 |
| R-10 | 哈希复查与 `os.replace`/`unlink` 之间存在本地并发修改窗口 | 同一用户下的其他进程若精确介入最终校验后的极短窗口，可能使事务覆盖或删除其最新修改 | 用户选择保留路径式事务；使用写前哈希、临时文件、同目录原子替换、rollback 前当前哈希、恢复材料哈希和状态机降低误操作风险，不宣称提供跨进程条件更新或操作系统级隔离 |
| R-11 | 路径式项目记忆存在本地并发替换（TOCTOU）窗口 | 同一 Windows 用户下的恶意进程若在静态检查与读取、替换、隔离或清理之间替换路径，应用层检查不能提供句柄级隔离 | 沿用用户对 Task 5/6 选择的路径式方案 B；检查目标及父链重解析点、操作前复核、同目录原子替换并禁止不安全状态覆盖，同时明确不把项目记忆描述为对抗本地恶意进程的安全边界 |
| R-12 | 工作区内测试代码不受进程级隔离 | pytest 以当前用户权限运行，启发式 capability 扫描可被字符串拼接、未列 API 或纯内置函数绕过 | 安全保证仅来自工作区围栏、动作级策略/HITL 和文件事务/回滚；完整隔离需要容器或系统级沙箱，列为非目标；仅在一次性或临时项目目录中使用 |
...
### C-02 临时 API Key 历史冲突（已解决）

早期历史曾包含额度受限的临时 Key；用户先后知情授权私有开发推送，并于 2026-08-12 授权清理。规范 GitHub/NJU `main` 已更新为精确重写后的历史，旧 GitHub 遗留分支已删除；fresh clone 的当前树和完整历史扫描均退出 0，AC-24 的仓库侧阻断已解除。GitHub cached views/internal PR refs 的永久清理由平台 Support 控制，不把规范 refs 已通过与平台缓存边界混为一谈。

### C-03 文档阶段在 main，编码阶段改用 worktree/PR（已解决）

早期文档和实现过程的提交结构均保留；历史清理只替换敏感模式并更新 commit SHA，没有压缩为单一新根提交。正式编码阶段的每个 PLAN task 已使用独立 branch + worktree，在测试和评审通过后通过 PR 合并回 `main`，因此此前流程冲突已解决。
...
本文件已完成占位符、一致性、范围和歧义自检，用户于 2026-08-08 明确回复“批准 SPEC”。SPEC 审阅门禁已经解除；本次批准允许：
...
`writing-plans` 已据此生成正式 `PLAN.md`。当前仍不得编写实现代码，也不得自行重写 Git 历史或提前创建实现分支/worktree；必须先完成不同类型智能体的冷启动验证，并对由此产生的 SPEC/PLAN 修改重新自检和批准。
