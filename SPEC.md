...
`77da924` 已包含额度受限的临时 Key，违反课程“当前树和历史均不得有凭据”的要求。用户决定保留现有历史、不轮换也不重写，并于 2026-08-11 在知情后明确授权把现有完整历史推送到指定 GitHub 仓库。该授权不等于满足课程凭据要求；公开发布或正式课程提交门禁仍必须执行历史扫描并如实失败，在用户改变授权前 AC-24 不可能通过。
