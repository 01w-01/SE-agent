...
- [ ] **Gate 2：在可舍弃 worktree 试做 Task 2 和 Task 5**

  明确要求陌生智能体：“遇到不确定处立即暂停提问，不要猜测；不要提交或合并”。试做时间控制在 1–2 小时。
...
- [ ] **Gate 4：修订并重新批准**
  ...
  若发现歧义，先修改 `SPEC.md`/`PLAN.md`，给出关键 diff，自检并由用户重新批准。冷启动 worktree 不合并，验证结束后安全移除。
